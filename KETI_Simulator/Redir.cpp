...
BOOL CRedirector::Open()
{
	...
	__try
	{
		...
		if (!LaunchChild(hStdoutWrite, hStdinRead, hStderrWrite))
			__leave;
	...
	__finally
	{
		if (!bOK)
		{
			...
			wchar_t wszMsg[256];
			MultiByteToWideChar(CP_ACP,MB_PRECOMPOSED,szMsg,strlen(szMsg),wszMsg,256);
			WriteStdError(wszMsg);
			DestroyHandle(hStdoutReadTmp);
			DestroyHandle(hStdoutWrite);
			DestroyHandle(hStderrWrite);
			DestroyHandle(hStdinWriteTmp);
...
BOOL CRedirector::Printf(char strInput[])
{
	if (!m_hStdinWrite)
		return FALSE;
	...
	//va_end(argList);
	//char strInput[] = "app list\r\n";

	DWORD dwWritten;
	return ::WriteFile(m_hStdinWrite, (LPCTSTR)strInput,
		strlen(strInput), &dwWritten, NULL);
}

BOOL CRedirector::LaunchChild(HANDLE hStdOut,
							  HANDLE hStdIn,
							  HANDLE hStdErr)
{
...
int CRedirector::RedirectStdout()
{
	...
	for (;;)
	{
		...
		wchar_t wszOutput[256] = {0,};
		MultiByteToWideChar(CP_ACP,MB_PRECOMPOSED,szOutput,strlen(szOutput),wszOutput,255);
		WriteStdOut(wszOutput);
	}
	...
	WriteStdError(_T("Read stdout pipe error\r\n"));
	return -1;		// os error
}
...
void CRedirector::WriteStdOut(LPCTSTR pszOutput)
{
	if (m_pWnd != NULL)
	{
		...
		if (nSize > 29000)
		{
			...
			nSize = m_pWnd->GetWindowTextLength();
		}
...
void CRedirector::WriteStdError(LPCTSTR pszError)
{
	WriteStdOut(_T("\r\nError message: "));
...
void CRedirector::KillProcess(CString strProcessName)
{
...
bool CRedirector::GetProcessModule(DWORD dwPID,CString sProcessName)
{ 
